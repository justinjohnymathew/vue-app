...
export default createStore({
  ...
  state: {
    currentUser: localStorage.getItem('currentUser') || '', // stores details of current user
  ...
  actions: {
    ...
    register ({ commit, state }, user) {
      return new Promise((resolve, reject) => {
        if (!state.usernames.includes(user.username)) {
    ...
    login ({ commit, state }, user) {
      return new Promise((resolve, reject) => {
        let userId = ''// stores array index of user
        ...
        if (userId !== -1) {
          ...
          if (state.users[userId].password === user.password) {
            ...
            commit('LOGIN', state.users[userId])
            resolve()
          } else {
